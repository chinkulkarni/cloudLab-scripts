...
if [ -f /local/startup_service_done ]; then
    ...
    # TODO: Start NFS server on rcnfs.

    # TODO: Mount hugepages on the NUMA node with the NIC (c6220).
...
if [ "$hostname" = "rcnfs" ]; then
    ...
    kernel_boot_params="default_hugepagesz=1G hugepagesz=1G hugepages=2"

    # TODO: Mount hugepages on the NUMA node with the NIC.
